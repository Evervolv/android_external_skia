...
bool SkImageRef_ashmem::pixelsAvailable() {
    // Quick test to see if the pixels are available.
    // If fPinned, pixels are guaranteed to be available.
    // If fRec.fFD != -1, means pixels were available before but may have
    // been unpinned in ashmem. For quick queries, assume that the pixels
    // are available if fRec.fFD != 1.
    return (fRec.fPinned) || (-1 != fRec.fFD);
}
